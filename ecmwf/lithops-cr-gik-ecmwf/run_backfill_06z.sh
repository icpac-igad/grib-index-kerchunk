#!/usr/bin/env bash
# ================================================================
# run_backfill_06z.sh — Month-by-month ECMWF 06Z parquet backfill
# ================================================================
#
...
#
# Usage:
#   bash run_backfill_06z.sh                          # Full: 2024-03 → 2026-01
#   bash run_backfill_06z.sh --from 2025-06            # Resume from 2025-06
#   bash run_backfill_06z.sh --from 2024-10 --to 2024-12
#   bash run_backfill_06z.sh --dry-run                 # Preview only
#   bash run_backfill_06z.sh --workers 20              # Custom worker count
#
# Background run (recommended for long backfills):
#   nohup bash run_backfill_06z.sh > logs/backfill_06z/run_full.log 2>&1 &
#   echo "PID: $!"
#
# Monitor:
#   tail -f logs/backfill_06z/summary.log
#   grep -E "^(OK|FAIL)" logs/backfill_06z/summary.log
# ================================================================
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help|-h)
            echo "Usage: bash run_backfill_06z.sh [--from YYYY-MM] [--to YYYY-MM] [--workers N] [--dry-run]"
            exit 0
            ;;
...
# ── Logging setup ───────────────────────────────────────────────────
LOG_DIR="$SCRIPT_DIR/logs/backfill_06z"
mkdir -p "$LOG_DIR"
...
for MONTH in $MONTHS; do
    ...
    MONTH_LOG="$LOG_DIR/backfill_06z_${MONTH}.log"
    MONTH_START=$(date +%s)
