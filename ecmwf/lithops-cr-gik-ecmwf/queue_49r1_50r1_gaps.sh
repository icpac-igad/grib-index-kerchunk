...
# 1) Wait for the 0p4 wave to finish (if a PID was given / can be found)
WAVE_PID="${WAVE_PID:-$(pgrep -f 'run_backfill_00z.sh --era 0p4' | head -1)}"
if [[ -n "${WAVE_PID:-}" ]]; then
  echo "[queue] waiting for 0p4 wave PID $WAVE_PID to finish ..."
  while kill -0 "$WAVE_PID" 2>/dev/null; do sleep 30; done
  echo "[queue] 0p4 wave finished; also draining any child run_lithops ..."
  while pgrep -f 'run_lithops_ecmwf.py' >/dev/null 2>&1; do sleep 20; done
else
  echo "[queue] no 0p4 wave PID found; assuming it is already done."
fi
