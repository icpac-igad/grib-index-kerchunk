...
# ///
"""
Validate GIK parquet references against Herbie for 11 random 2025 dates.

Reads pre-built parquet files from the 2025 nested GCS path:
    gs://gik-ecmwf-aws-tf/run_par_ecmwf/2025/{MM}/{YYYYMMDD}/00z/
...
Output directory: validation_gik_vs_herbie_2025/

Usage:
    uv run validate_gik_vs_herbie_2025.py
    uv run validate_gik_vs_herbie_2025.py --dry-run
    uv run validate_gik_vs_herbie_2025.py --max-members 5
    uv run validate_gik_vs_herbie_2025.py --dates 20250115,20250406
"""
...
# Output
OUTPUT_DIR = SCRIPT_DIR / "validation_gik_vs_herbie_2025"
...
def generate_11_dates(seed: int = 42) -> list[date]:
    """Pick one random date per month for January–November 2025 (11 dates)."""
...
def list_member_parquets(date_str: str, max_members: int = None) -> list[str]:
    """List parquet files for a date on GCS (2025 nested path)."""
    month = date_str[4:6]
...
def member_key_from_filename(path: str) -> str:
    ...
    '2025040600z-control.parquet' -> 'control'
    '2025040600z-ens_01.parquet'  -> 'ens01'
    """
...
def plot_scatter_all(all_gik_means, all_herbie_means, output_dir: Path):
    ...
    ax.set_title(f"GIK vs Herbie — All 11 dates (2025) at T+48h\nr = {r:.8f}  N = {len(all_g):,}",
                 fontweight="bold")
    ax.legend()
    ax.set_aspect("equal")
    plt.tight_layout()
...
def setup_logging(out_dir: Path) -> logging.Logger:
    ...
    logger = logging.getLogger("validate_gik_herbie_2025")
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate GIK vs Herbie TP for 11 random 2025 dates")
    ...
    if args.dates:
        ...
        dates = generate_11_dates(seed=args.seed)

    if args.dry_run:
        print(f"11 random dates (seed={args.seed}):")
        for d in dates:
    ...
    logger.info("=" * 70)
    logger.info("GIK vs Herbie Validation — 2025 parquets")
    logger.info("=" * 70)
