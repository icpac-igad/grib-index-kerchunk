...
# ///
"""
ECMWF EA Total Precipitation — Materialized Icechunk Store on source.coop
==========================================================================

Creates a materialized Icechunk store for ECMWF IFS ensemble total precipitation
(tp) over the East Africa / ICPAC region, writing to source.coop S3 instead of
GCS.  Adapted from ecmwf_ea_tp_icechunk.py.

Pipeline:
  HuggingFace parquets (E4DRR/gik-ecmwf-par)
    → Coiled/Dask workers fetch GRIB bytes from S3 (ecmwf-forecasts, anonymous)
    → Decode with gribberish, subset to East Africa (157×145)
    → Coordinator writes to Icechunk store on source.coop S3
...
The 1-hour credential window means you should process ~1 month per session.
The script includes a credential timeout guard that stops before token expiry.
Resume detection via commit messages lets you restart with fresh credentials.
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("ecmwf_ea_tp_source_coop.log"),
...
def load_s3_credentials():
    ...
    if env_path.exists():
        # Parse shell export format: strip 'export ' prefix and quotes
...
def make_s3_storage(creds=None, anonymous=False):
    """Create icechunk S3 storage for source.coop.
...
def init_store(args):
    """Create empty template Icechunk store on source.coop.
    ...
    logger.info("=" * 60)
    logger.info("INIT: Creating ECMWF EA tp Icechunk store on source.coop")
    logger.info("=" * 60)
    start = time.time()
    ...
    # Set up Icechunk store on source.coop S3
    if args.local:
        ...
        storage = make_s3_storage(creds=creds)
...
def fill_store(args):
    ...
    The coordinator writes to the source.coop Icechunk store using STS
    credentials from .env.  A timeout guard stops processing before the
    1-hour token expires so you can refresh and resume.
    """
    import coiled
    import distributed
    import icechunk
    ...
    logger.info("=" * 60)
    logger.info("FILL: Populating ECMWF EA tp store on source.coop")
    logger.info("=" * 60)
    ...
    # Open target Icechunk store on source.coop
    if args.local:
        target_storage = icechunk.local_filesystem_storage(path=args.local)
    else:
        creds = load_s3_credentials()
        logger.info(f"  Target: s3://{S3_BUCKET}/{S3_PREFIX}/")
        target_storage = make_s3_storage(creds=creds)
    ...
    cluster = coiled.Cluster(
        name=f"ecmwf-ea-tp-sc-{int(time.time()) % 10000}",
        n_workers=n_workers,
    ...
    for batch_start in range(0, len(remaining), COMMIT_BATCH):
        ...
        # Collect results and write+commit each date individually
        date_members = {}
        date_expected = {}
        date_fail_count = {}
    ...
    results_path = f"ecmwf_ea_tp_source_coop_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def verify_store(args):
    """Inspect and verify the Icechunk store on source.coop.
    ...
    logger.info("=" * 60)
    logger.info("VERIFY: Inspecting ECMWF EA tp store on source.coop")
    logger.info("=" * 60)
    ...
    if args.local:
        ...
        storage = make_s3_storage(anonymous=True)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="ECMWF EA TP — Materialized Icechunk Store on source.coop",
    )
