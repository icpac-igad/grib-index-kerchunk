#!/usr/bin/env python3
# /// script
# requires-python = ">=3.10"
# dependencies = [
#     "numpy",
#     "pandas",
#     "xarray",
#     "dask",
#     "fsspec",
#     "s3fs",
#     "pyarrow",
#     "gribberish",
#     "coiled",
#     "distributed",
#     "bokeh>=3.1.0",
#     "zarr<3",
# ]
# ///
"""
ECMWF EA Total Precipitation — Plain Zarr Store on source.coop
===============================================================

Creates a materialized Zarr store for ECMWF IFS ensemble total precipitation
(tp) over the East Africa / ICPAC region, writing plain Zarr (no Icechunk)
to source.coop S3.

This avoids Icechunk's transaction/commit layer which has issues with:
- STS credential rotation (1-hour token expiry)
- Cluster restart between batches (stale repo state)
- Per-date commits across rotating Coiled clusters

Instead, each date's data is written as a region slice into a plain Zarr store
on S3 via s3fs. Each chunk is an independent S3 object — no transactions,
no commit conflicts, fully resumable.

Pipeline:
  HuggingFace parquets (E4DRR/gik-ecmwf-par)
    -> Coiled/Dask workers fetch GRIB bytes from S3 (ecmwf-forecasts, anonymous)
    -> Decode with gribberish, subset to East Africa (157x145)
    -> Coordinator writes plain Zarr to source.coop S3

Credentials:
  Workers:      None needed (HF is public HTTPS, ECMWF S3 is anonymous)
  Coordinator:  AWS STS temporary credentials from .env file (1-hour lifetime)

Subcommands:
  init   -- Create empty template Zarr store on source.coop
  fill   -- Populate with real data using Dask/Coiled workers
  verify -- Inspect store contents (anonymous read, no credentials needed)

Usage:
    # 1. Create .env with fresh STS credentials
    cat > .env << 'EOF'
    export AWS_ACCESS_KEY_ID="ASIA..."
    export AWS_SECRET_ACCESS_KEY="..."
    export AWS_SESSION_TOKEN="..."
    export AWS_DEFAULT_REGION="us-west-2"
    EOF

    # 2. Init (one-time)
    uv run ecmwf_ea_tp_source_coop_zarr.py init \\
        --start-date 20240301 --end-date 20251231

    # 3. Fill month by month (refresh .env credentials between runs)
    uv run ecmwf_ea_tp_source_coop_zarr.py fill \\
        --start-date 20240301 --end-date 20240401 \\
        --n-workers 50

    # 4. Verify (anonymous, no credentials needed)
    uv run ecmwf_ea_tp_source_coop_zarr.py verify

Author: ICPAC GIK Team
"""

import json
import logging
import os
import tempfile
import time
import warnings
from datetime import datetime
from pathlib import Path
from typing import List

import numpy as np
import pandas as pd

warnings.filterwarnings("ignore")
os.environ["AWS_NO_SIGN_REQUEST"] = "YES"

logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - %(levelname)s - %(message)s",
    handlers=[
        logging.FileHandler("ecmwf_ea_tp_source_coop_zarr.log"),
        logging.StreamHandler(),
    ],
)
logger = logging.getLogger(__name__)

# ─── Constants ──────────────────────────────────────────────────────────────

SCRIPT_DIR = Path(__file__).resolve().parent

# Source.coop S3 target (plain Zarr, not Icechunk)
S3_BUCKET = "us-west-2.opendata.source.coop"
S3_PREFIX = "e4drr-project/forecasts/ecmwf_ea_tp_zarr"
S3_REGION = "us-west-2"

# HuggingFace parquet source
HF_REPO = "E4DRR/gik-ecmwf-par"
HF_BASE_URL = (
    "https://huggingface.co/datasets/E4DRR/gik-ecmwf-par/resolve/main/run_par_ecmwf"
)
HF_COMBINED_URL = (
    "https://huggingface.co/datasets/E4DRR/gik-ecmwf-par/resolve/main/combined"
)

# ECMWF forecast lead times (00z run): 0-144h @3h + 150-168h @6h
LEAD_TIME_HOURS_3H = list(range(0, 145, 3))   # 49 steps
LEAD_TIME_HOURS_6H = [150, 156, 162, 168]     # 4 steps
LEAD_TIME_HOURS = LEAD_TIME_HOURS_3H + LEAD_TIME_HOURS_6H  # 53 steps

# Ensemble members: 1 control + 50 ensemble
MEMBER_IDS = ["control"] + [f"ens_{i:02d}" for i in range(1, 51)]  # 51 members

# ECMWF global grid
ECMWF_GRID_SHAPE = (721, 1440)
ECMWF_LATS = np.linspace(90, -90, 721)
ECMWF_LONS = np.linspace(-180, 179.75, 1440)

# East Africa / ICPAC bounding box
LAT_MIN, LAT_MAX = -14, 25
LON_MIN, LON_MAX = 19, 55

# Precompute EA spatial indices
_lat_mask = (ECMWF_LATS >= LAT_MIN) & (ECMWF_LATS <= LAT_MAX)
_lon_mask = (ECMWF_LONS >= LON_MIN) & (ECMWF_LONS <= LON_MAX)
LAT_INDICES = np.where(_lat_mask)[0]
LON_INDICES = np.where(_lon_mask)[0]
EA_LATS = ECMWF_LATS[LAT_INDICES[0] : LAT_INDICES[-1] + 1]
EA_LONS = ECMWF_LONS[LON_INDICES[0] : LON_INDICES[-1] + 1]
N_LAT = len(EA_LATS)  # 157
N_LON = len(EA_LONS)  # 145
N_STEPS = len(LEAD_TIME_HOURS)  # 53
N_MEMBERS = len(MEMBER_IDS)  # 51

# Chunk shape: one init_date, one member, all lead times and spatial dims
CHUNK_SHAPE = (1, 1, N_STEPS, N_LAT, N_LON)

# Credential timeout: stop 15 minutes before the 1-hour STS token expires
CREDENTIAL_TIMEOUT_SECONDS = 45 * 60  # 45 minutes


# ─── Credential helpers ───────────────────────────────────────────────────


def load_s3_credentials():
    """Load source.coop S3 credentials from .env file.

    The .env file uses shell export syntax:
        export AWS_ACCESS_KEY_ID="ASIA..."
        export AWS_SECRET_ACCESS_KEY="..."
        export AWS_SESSION_TOKEN="..."
        export AWS_DEFAULT_REGION="us-west-2"

    Returns dict with keys: access_key_id, secret_access_key, session_token.
    """
    env_path = SCRIPT_DIR / ".env"
    if env_path.exists():
        for line in env_path.read_text().splitlines():
            line = line.strip()
            if not line or line.startswith("#"):
                continue
            if line.startswith("export "):
                line = line[7:]
            if "=" in line:
                key, _, value = line.partition("=")
                key = key.strip()
                value = value.strip().strip('"').strip("'")
                os.environ[key] = value

    access_key = os.getenv("AWS_ACCESS_KEY_ID")
    secret_key = os.getenv("AWS_SECRET_ACCESS_KEY")
    session_token = os.getenv("AWS_SESSION_TOKEN")

    if not access_key or not secret_key:
        raise RuntimeError(
            "AWS credentials not found. Create a .env file with:\n"
            '  export AWS_ACCESS_KEY_ID="ASIA..."\n'
            '  export AWS_SECRET_ACCESS_KEY="..."\n'
            '  export AWS_SESSION_TOKEN="..."\n'
            '  export AWS_DEFAULT_REGION="us-west-2"'
        )

    return {
        "access_key_id": access_key,
        "secret_access_key": secret_key,
        "session_token": session_token,
    }


def make_s3fs(creds=None, anonymous=False):
    """Create s3fs filesystem for source.coop.

    Args:
        creds: dict from load_s3_credentials(), or None for anonymous.
        anonymous: if True, open for public read (no credentials).
    """
    import s3fs

    if anonymous:
        return s3fs.S3FileSystem(anon=True)

    if creds is None:
        creds = load_s3_credentials()

    return s3fs.S3FileSystem(
        key=creds["access_key_id"],
        secret=creds["secret_access_key"],
        token=creds.get("session_token"),
    )


def make_zarr_store(creds=None, anonymous=False):
    """Create a Zarr-compatible S3Map store for source.coop.

    Returns an fsspec mapping that xarray can write to / read from directly.
    """
    import s3fs

    fs = make_s3fs(creds=creds, anonymous=anonymous)
    return s3fs.S3Map(root=f"{S3_BUCKET}/{S3_PREFIX}", s3=fs)


# ─── Date helpers ───────────────────────────────────────────────────────────


def build_date_list(start_date: str, end_date: str) -> List[str]:
    """Build list of YYYYMMDD date strings between start and end (inclusive)."""
    dates = pd.date_range(start_date, end_date, freq="D")
    return [d.strftime("%Y%m%d") for d in dates]


# ─── Phase 1: init ─────────────────────────────────────────────────────────


def init_store(args):
    """Create empty template Zarr store on source.coop.

    Writes the Zarr metadata (coordinates, encoding, attributes) and fills
    the data variable with NaN. This is a one-time operation.
    """
    import dask.array as da
    import xarray as xr

    logger.info("=" * 60)
    logger.info("INIT: Creating ECMWF EA tp Zarr store on source.coop")
    logger.info("=" * 60)
    start = time.time()

    dates = build_date_list(args.start_date, args.end_date)
    n_dates = len(dates)
    logger.info(f"  Dates: {n_dates} ({dates[0]} to {dates[-1]})")
    logger.info(f"  Members: {N_MEMBERS}")
    logger.info(f"  Lead times: {N_STEPS}")
    logger.info(f"  Spatial: {N_LAT} lat x {N_LON} lon")

    # Coordinate arrays
    init_date = pd.to_datetime(dates).values.astype("datetime64[ns]")
    lead_time = np.array(LEAD_TIME_HOURS, dtype=np.int32)
    member = np.array(MEMBER_IDS, dtype="U10")

    shape = (n_dates, N_MEMBERS, N_STEPS, N_LAT, N_LON)
    size_gb = np.prod(shape) * 4 / (1024**3)
    logger.info(f"  Shape: {shape}")
    logger.info(f"  Uncompressed size: {size_gb:.1f} GiB")
    logger.info(f"  Chunk shape: {CHUNK_SHAPE}")

    # Create lazy template (no memory allocation)
    template = xr.Dataset(
        {
            "tp": (
                ("init_date", "member", "lead_time", "lat", "lon"),
                da.full(shape, np.nan, chunks=CHUNK_SHAPE, dtype=np.float32),
                {
                    "long_name": "Total precipitation",
                    "units": "m",
                    "source": "ECMWF IFS ensemble (enfo)",
                },
            ),
        },
        coords={
            "init_date": ("init_date", init_date),
            "member": ("member", member),
            "lead_time": ("lead_time", lead_time, {"units": "hours"}),
            "lat": ("lat", EA_LATS, {"units": "degrees_north"}),
            "lon": ("lon", EA_LONS, {"units": "degrees_east"}),
        },
        attrs={
            "title": "ECMWF IFS Ensemble TP — East Africa Subset",
            "source": "GIK parquet references -> S3 GRIB byte-range reads",
            "institution": "ICPAC",
            "region": "East Africa (ICPAC)",
            "lat_range": f"{LAT_MIN} to {LAT_MAX}",
            "lon_range": f"{LON_MIN} to {LON_MAX}",
            "storage": f"s3://{S3_BUCKET}/{S3_PREFIX}/",
            "format": "Zarr v2 (plain, no Icechunk)",
        },
    )
    logger.info(f"  Template:\n{template}")

    # Write to S3
    if args.local:
        logger.info(f"  Using local storage: {args.local}")
        store = args.local
    else:
        creds = load_s3_credentials()
        logger.info(f"  Target: s3://{S3_BUCKET}/{S3_PREFIX}/")
        store = make_zarr_store(creds=creds)

    template.to_zarr(
        store,
        mode="w",
        compute=False,
        encoding={
            "tp": {
                "chunks": CHUNK_SHAPE,
                "dtype": "float32",
            },
        },
        consolidated=True,
    )
    elapsed = time.time() - start

    logger.info("=" * 60)
    logger.info("INIT COMPLETE")
    logger.info(f"  Shape: {shape}")
    logger.info(f"  Chunks: {CHUNK_SHAPE}")
    logger.info(f"  Target: s3://{S3_BUCKET}/{S3_PREFIX}/")
    logger.info(f"  Time: {elapsed:.1f}s")
    logger.info("=" * 60)


# ─── Phase 2: fill ─────────────────────────────────────────────────────────


def fill_store(args):
    """Fill store with real data from HuggingFace parquets -> S3 GRIB reads.

    Workers fetch parquets from HuggingFace (public) and GRIB bytes from
    ECMWF S3 (anonymous) -- no credentials needed on workers.

    The coordinator writes to plain Zarr on source.coop S3 using STS
    credentials from .env. No Icechunk commits -- each region write is
    an independent S3 put_object, fully resumable.
    """
    import coiled
    import distributed
    import xarray as xr

    logger.info("=" * 60)
    logger.info("FILL: Populating ECMWF EA tp Zarr store on source.coop")
    logger.info("=" * 60)
    overall_start = time.time()
    session_start = time.time()

    dates = build_date_list(args.start_date, args.end_date)
    n_dates = len(dates)
    logger.info(f"  Dates: {n_dates} ({dates[0]} to {dates[-1]})")

    # Open target Zarr store
    if args.local:
        store = args.local
    else:
        creds = load_s3_credentials()
        logger.info(f"  Target: s3://{S3_BUCKET}/{S3_PREFIX}/")
        store = make_zarr_store(creds=creds)

    # Resume detection: check which date indices already have non-NaN data
    # by reading a single pixel from tp for each date
    logger.info("  Checking for already-filled dates...")
    try:
        ds_check = xr.open_zarr(store, consolidated=True)
        # Sample first member, first step, first lat/lon
        sample = ds_check["tp"].isel(member=0, lead_time=0, lat=0, lon=0).values
        filled_mask = ~np.isnan(sample)
        n_filled = int(filled_mask.sum())
        ds_check.close()
        logger.info(f"  Already filled: {n_filled}/{n_dates} dates")
    except Exception as e:
        logger.warning(f"  Could not check existing data: {e}")
        filled_mask = np.zeros(n_dates, dtype=bool)
        n_filled = 0

    remaining = [(i, d) for i, d in enumerate(dates) if not filled_mask[i]]
    if not remaining:
        logger.info("  All dates already filled!")
        return
    logger.info(f"  Remaining: {len(remaining)} dates")

    timeout = args.credential_timeout
    logger.info(f"  Credential timeout: {timeout}s ({timeout/60:.0f} min)")

    # Launch Coiled cluster
    n_workers = args.n_workers
    cluster = coiled.Cluster(
        name=f"ecmwf-ea-tp-zarr-{int(time.time()) % 10000}",
        n_workers=n_workers,
        worker_vm_types="e2-standard-4",
        package_sync=True,
        region="europe-west1",
        idle_timeout="30 minutes",
    )
    client = distributed.Client(cluster)
    client.wait_for_workers(n_workers=min(10, n_workers), timeout=600)
    logger.info(f"  Cluster ready: {client.dashboard_link}")

    # ── Worker function: one Dask task per (date, member) ──
    # Workers need NO credentials -- HF is public HTTPS, ECMWF S3 is anonymous.

    def read_member_tp_ea(date_str, member_id, lead_time_hours,
                          hf_base_url, grid_shape, lat_idx_start, lat_idx_end,
                          lon_idx_start, lon_idx_end,
                          hf_combined_url=None):
        """Process one (date, member): fetch 53 tp steps from S3.

        Returns dict with member data array of shape (53, n_lat, n_lon) ~4.6 MB.
        """
        import json
        import os
        import tempfile
        import warnings
        from concurrent.futures import ThreadPoolExecutor, as_completed

        import fsspec
        import numpy as np
        import pandas as pd

        warnings.filterwarnings("ignore")
        os.environ["AWS_NO_SIGN_REQUEST"] = "YES"

        try:
            import gribberish
            has_gribberish = True
        except ImportError:
            has_gribberish = False

        n_steps = len(lead_time_hours)
        n_lat = lat_idx_end - lat_idx_start
        n_lon = lon_idx_end - lon_idx_start

        year = date_str[:4]
        month = date_str[4:6]

        # Try combined parquet with predicate pushdown first
        df = None
        if hf_combined_url:
            try:
                combined_url = f"{hf_combined_url}/ecmwf_gik_00z.parquet"
                cache_dir = os.path.join(tempfile.gettempdir(), "gik_combined_cache")
                os.makedirs(cache_dir, exist_ok=True)
                cache_path = os.path.join(cache_dir, "ecmwf_gik_00z.parquet")

                if not os.path.exists(cache_path):
                    import urllib.request
                    urllib.request.urlretrieve(combined_url, cache_path)

                import pyarrow.parquet as pq
                table = pq.read_table(
                    cache_path,
                    filters=[
                        ("date", "==", date_str),
                        ("member", "==", member_id),
                    ],
                )
                df = table.to_pandas()
                del table
            except Exception:
                df = None

        # Fallback: per-member parquet URL
        if df is None or df.empty:
            parquet_url = (
                f"{hf_base_url}/{year}/{month}/{date_str}/00z/"
                f"{date_str}00z-{member_id}.parquet"
            )
            df = pd.read_parquet(parquet_url)

        # Build zstore dict
        zstore = {}
        for _, row in df.iterrows():
            key = row["key"]
            value = row["value"]
            if isinstance(value, bytes):
                try:
                    decoded = value.decode("utf-8")
                    if decoded.startswith("[") or decoded.startswith("{"):
                        value = json.loads(decoded)
                    else:
                        value = decoded
                except Exception:
                    pass
            elif isinstance(value, str):
                if value.startswith("[") or value.startswith("{"):
                    try:
                        value = json.loads(value)
                    except Exception:
                        pass
            zstore[key] = value
        del df

        member_key = member_id.replace("_", "")

        # Collect (step_idx, ref) pairs for tp
        step_refs = []
        for s_idx, step_h in enumerate(lead_time_hours):
            for pattern in [
                f"step_{step_h:03d}/tp/sfc/{member_key}/0.0.0",
                f"step_{step_h:03d}/tp/sfc/0.0.0",
                f"step_{step_h:03d}/tp/surface/{member_key}/0.0.0",
            ]:
                if pattern in zstore:
                    val = zstore[pattern]
                    if isinstance(val, list) and len(val) >= 3:
                        step_refs.append((s_idx, val))
                        break
        del zstore

        member_data = np.full((n_steps, n_lat, n_lon), np.nan, dtype=np.float32)
        if not step_refs:
            return {"date_str": date_str, "member_id": member_id, "data": member_data}

        s3_fs = fsspec.filesystem("s3", anon=True)

        def _fetch_one_step(s_idx, ref):
            url, offset, length = ref[0], ref[1], ref[2]
            if not url.endswith(".grib2"):
                url = url + ".grib2"
            with s3_fs.open(url, "rb") as f:
                f.seek(offset)
                grib_bytes = f.read(length)

            arr = None
            if has_gribberish:
                try:
                    flat = gribberish.parse_grib_array(grib_bytes, 0)
                    arr = flat.reshape(grid_shape)
                except Exception:
                    pass
            if arr is None:
                import xarray as xr
                with tempfile.NamedTemporaryFile(
                    delete=False, suffix=".grib2"
                ) as tmp:
                    tmp.write(grib_bytes)
                    tmp_path = tmp.name
                try:
                    ds = xr.open_dataset(tmp_path, engine="cfgrib")
                    arr = ds[list(ds.data_vars)[0]].values.copy()
                    ds.close()
                finally:
                    os.unlink(tmp_path)

            ea_arr = arr[lat_idx_start:lat_idx_end,
                         lon_idx_start:lon_idx_end].astype(np.float32)
            return s_idx, ea_arr

        # Fetch all 53 steps in parallel (8 threads -- I/O bound S3 reads)
        with ThreadPoolExecutor(max_workers=8) as pool:
            futs = {
                pool.submit(_fetch_one_step, s_idx, ref): s_idx
                for s_idx, ref in step_refs
            }
            for fut in as_completed(futs):
                try:
                    s_idx, ea_arr = fut.result()
                    member_data[s_idx] = ea_arr
                except Exception:
                    pass

        return {"date_str": date_str, "member_id": member_id, "data": member_data}

    # Precompute EA spatial slice indices
    lat_idx_start = int(LAT_INDICES[0])
    lat_idx_end = int(LAT_INDICES[-1]) + 1
    lon_idx_start = int(LON_INDICES[0])
    lon_idx_end = int(LON_INDICES[-1]) + 1

    # Process in batches
    COMMIT_BATCH = args.commit_batch
    total_written = 0
    total_failed = 0
    failed_dates = []
    timed_out = False

    for batch_start in range(0, len(remaining), COMMIT_BATCH):
        # ── Credential timeout guard ──
        elapsed_session = time.time() - session_start
        if elapsed_session > timeout:
            logger.warning(
                f"  Credential timeout ({timeout}s) reached after "
                f"{elapsed_session:.0f}s. Stopping to avoid expired token. "
                f"Re-run with fresh .env credentials to resume."
            )
            timed_out = True
            break

        batch = remaining[batch_start : batch_start + COMMIT_BATCH]
        batch_idx_min = batch[0][0]
        batch_idx_max = batch[-1][0]
        n_tasks = len(batch) * N_MEMBERS
        logger.info(
            f"  Batch: date indices {batch_idx_min}-{batch_idx_max} "
            f"({len(batch)} dates x {N_MEMBERS} members = {n_tasks} tasks, "
            f"{total_written}/{len(remaining)} dates done, "
            f"{elapsed_session:.0f}s/{timeout}s credential budget)"
        )

        # Submit one Dask task per (date, member)
        futures = {}
        for date_idx, date_str in batch:
            for m_idx, member_id in enumerate(MEMBER_IDS):
                future = client.submit(
                    read_member_tp_ea,
                    date_str,
                    member_id,
                    LEAD_TIME_HOURS,
                    HF_BASE_URL,
                    ECMWF_GRID_SHAPE,
                    lat_idx_start, lat_idx_end,
                    lon_idx_start, lon_idx_end,
                    HF_COMBINED_URL,
                    key=f"d{date_idx}-m{m_idx:02d}",
                )
                futures[future] = (date_idx, date_str, m_idx)

        # Collect results and write each date as a region slice
        date_members = {}
        date_expected = {}
        date_fail_count = {}

        batch_ok = 0
        batch_fail = 0
        tasks_done = 0

        for future in distributed.as_completed(futures):
            date_idx, date_str, m_idx = futures[future]
            try:
                result = future.result()
                date_members.setdefault(date_idx, {})[m_idx] = result["data"]
                del result
            except Exception:
                date_fail_count[date_idx] = date_fail_count.get(date_idx, 0) + 1

            date_expected[date_idx] = date_expected.get(date_idx, 0) + 1
            tasks_done += 1

            if tasks_done % 100 == 0:
                elapsed_session = time.time() - session_start
                logger.info(
                    f"    Progress: {tasks_done}/{n_tasks} member-tasks done "
                    f"({elapsed_session:.0f}s/{timeout}s)"
                )

            # Check if this date is complete (all 51 members received)
            if date_expected[date_idx] == N_MEMBERS:
                members = date_members.pop(date_idx, {})
                n_ok = len(members)
                n_fail = date_fail_count.get(date_idx, 0)

                if n_ok == 0:
                    batch_fail += 1
                    total_failed += 1
                    failed_dates.append(date_str)
                    logger.error(
                        f"    Date {date_idx} ({date_str}) FAILED: 0/{N_MEMBERS} members"
                    )
                    continue

                # Assemble (51, 53, 157, 145) and write as region slice
                data = np.full(
                    (N_MEMBERS, N_STEPS, N_LAT, N_LON), np.nan, dtype=np.float32
                )
                for m_i, member_data in members.items():
                    data[m_i] = member_data
                del members

                try:
                    # Re-create store for each write to avoid stale S3 connections
                    if args.local:
                        write_store = args.local
                    else:
                        write_store = make_zarr_store(creds=creds)

                    ds_write = xr.Dataset({
                        "tp": (
                            ("init_date", "member", "lead_time", "lat", "lon"),
                            data[np.newaxis],  # (1, 51, 53, 157, 145)
                        ),
                    })
                    ds_write.to_zarr(
                        write_store,
                        region={"init_date": slice(date_idx, date_idx + 1)},
                    )
                    del data

                    batch_ok += 1
                    total_written += 1
                    logger.info(
                        f"    Written date {date_idx} ({date_str}) "
                        f"[{n_ok}/{N_MEMBERS} members, {n_fail} failed] "
                        f"({total_written}/{len(remaining)} total)"
                    )
                except Exception as e:
                    batch_fail += 1
                    total_failed += 1
                    failed_dates.append(date_str)
                    logger.error(
                        f"    Date {date_idx} ({date_str}) WRITE FAILED: {e}"
                    )

    client.close()
    cluster.close()

    elapsed = time.time() - overall_start
    logger.info("=" * 60)
    logger.info("FILL COMPLETE" + (" (timed out -- resume with fresh creds)" if timed_out else ""))
    logger.info(f"  Dates written: {total_written}/{n_dates}")
    logger.info(f"  Failed: {total_failed} -- {failed_dates[:20]}")
    logger.info(f"  Time: {elapsed / 60:.1f} min")
    logger.info(f"  Store: s3://{S3_BUCKET}/{S3_PREFIX}/")
    logger.info("=" * 60)

    results = {
        "status": "timed_out" if timed_out else ("success" if not failed_dates else "partial"),
        "dates_written": total_written,
        "dates_total": n_dates,
        "failed_dates": failed_dates,
        "timed_out": timed_out,
        "elapsed_min": elapsed / 60,
    }
    results_path = f"ecmwf_ea_tp_zarr_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    with open(results_path, "w") as f:
        json.dump(results, f, indent=2, default=str)
    logger.info(f"  Results: {results_path}")


# ─── Phase 3: verify ───────────────────────────────────────────────────────


def verify_store(args):
    """Inspect and verify the Zarr store on source.coop.

    Uses anonymous read -- no credentials needed.
    """
    import xarray as xr

    logger.info("=" * 60)
    logger.info("VERIFY: Inspecting ECMWF EA tp Zarr store on source.coop")
    logger.info("=" * 60)

    if args.local:
        store = args.local
    else:
        logger.info(f"  Opening: s3://{S3_BUCKET}/{S3_PREFIX}/")
        store = make_zarr_store(anonymous=True)

    ds = xr.open_zarr(store, consolidated=True)

    logger.info(f"\nDataset:\n{ds}")
    logger.info(f"\nDimensions: {dict(ds.sizes)}")

    for dim in ["init_date", "member", "lead_time", "lat", "lon"]:
        if dim in ds.dims:
            vals = ds[dim].values
            logger.info(f"  {dim}: {ds.sizes[dim]} [{vals[0]} .. {vals[-1]}]")

    for var in ds.data_vars:
        da = ds[var]
        logger.info(f"\nVariable '{var}': dtype={da.dtype}, shape={da.shape}")
        if hasattr(da, "encoding") and "chunks" in da.encoding:
            logger.info(f"  chunks: {da.encoding['chunks']}")

    # Count filled dates
    if "tp" in ds.data_vars:
        sample = ds["tp"].isel(member=0, lead_time=0, lat=0, lon=0).values
        n_filled = int(np.count_nonzero(~np.isnan(sample)))
        n_total = len(sample)
        logger.info(f"\n  Filled dates: {n_filled}/{n_total} ({100*n_filled/n_total:.1f}%)")

    # Spot-check: load one date+member
    if args.spot_check and "tp" in ds.data_vars:
        # Find first non-NaN date
        first_filled = np.argmax(~np.isnan(sample))
        if not np.isnan(sample[first_filled]):
            logger.info(f"\nSpot-check: loading date index {first_filled}, first member...")
            try:
                spot = ds["tp"].isel(init_date=int(first_filled), member=0).load()
                n_valid = int((~np.isnan(spot.values)).sum())
                n_total_px = spot.values.size
                pct = 100 * n_valid / n_total_px if n_total_px else 0
                logger.info(f"  Valid (non-NaN): {n_valid}/{n_total_px} ({pct:.1f}%)")
                if n_valid > 0:
                    vals = spot.values[~np.isnan(spot.values)]
                    logger.info(f"  Min: {float(vals.min()):.6f}")
                    logger.info(f"  Max: {float(vals.max()):.6f}")
                    logger.info(f"  Mean: {float(vals.mean()):.6f}")
            except Exception as e:
                logger.error(f"  Spot-check failed: {e}")
        else:
            logger.info("\n  No filled dates found for spot-check.")

    ds.close()
    logger.info("\nVerification complete.")


# ─── CLI ────────────────────────────────────────────────────────────────────


def main():
    import argparse

    parser = argparse.ArgumentParser(
        description="ECMWF EA TP — Plain Zarr Store on source.coop",
    )
    sub = parser.add_subparsers(dest="command")

    # Common args
    common = argparse.ArgumentParser(add_help=False)
    common.add_argument("--local", type=str, default=None,
                        help="Local filesystem path (overrides source.coop S3)")

    # ── init ──
    p_init = sub.add_parser("init", parents=[common],
                            help="Create empty template Zarr store")
    p_init.add_argument("--start-date", type=str, default="20240301",
                        help="Start date YYYYMMDD")
    p_init.add_argument("--end-date", type=str, default="20251231",
                        help="End date YYYYMMDD")

    # ── fill ──
    p_fill = sub.add_parser("fill", parents=[common],
                            help="Fill store from HF parquets + S3 GRIB reads")
    p_fill.add_argument("--start-date", type=str, default="20240301")
    p_fill.add_argument("--end-date", type=str, default="20251231")
    p_fill.add_argument("--n-workers", type=int, default=20)
    p_fill.add_argument("--commit-batch", type=int, default=20,
                        help="Number of dates per processing batch")
    p_fill.add_argument("--credential-timeout", type=int,
                        default=CREDENTIAL_TIMEOUT_SECONDS,
                        help="Stop before this many seconds to avoid expired "
                             "STS token (default: 2700 = 45 min)")

    # ── verify ──
    p_verify = sub.add_parser("verify", parents=[common],
                              help="Inspect store contents (anonymous read)")
    p_verify.add_argument("--spot-check", action="store_true", default=True)
    p_verify.add_argument("--no-spot-check", action="store_false",
                          dest="spot_check")

    args = parser.parse_args()

    if args.command == "init":
        init_store(args)
    elif args.command == "fill":
        fill_store(args)
    elif args.command == "verify":
        verify_store(args)
    else:
        parser.print_help()


if __name__ == "__main__":
    main()
