...
month + run hour into a single parquet and uploads to a separate HF path:
    run_par_ecmwf_agg/{YYYY}/{MM}/{RUN}z.parquet

For 2024 this produces 12 months x 4 runs = 48 aggregated parquet files.
Each row gets 'date' and 'source_file' columns added for provenance.
...
Usage:
    ...
    uv run upload_parquets_to_hf.py --aggregate --year 2024   # aggregate by month+run
    uv run upload_parquets_to_hf.py --aggregate --run 00 --dry-run  # preview 00z aggregation
    uv run upload_parquets_to_hf.py --catalog                  # build & upload catalog index
    uv run upload_parquets_to_hf.py --catalog --dry-run        # preview catalog without uploading
"""
...
def aggregate_month(fs, api, year: str, month: str, run_hour: str,
    ...
    reads them with pyarrow, adds 'date' and 'source_file' columns, concatenates
    into one table, and uploads as:
    ...
    # Collect all parquet paths with their date
    ...
    # Download, read, concatenate, upload
    tmpdir = tempfile.mkdtemp()
    try:
        tables = []
...
def main():
    ...
    parser.add_argument("--catalog", action="store_true",
                        help="Build a lightweight catalog/index parquet listing "
                             "all available dates, runs, and members")
    parser.add_argument("--aggregate", action="store_true",
                        help="Aggregate parquets by month+run into single files "
                             "and upload to run_par_ecmwf_agg/ on HF")
    ...
    # ── Catalog mode: build index and exit ──
    if args.catalog:
    ...
    # Determine run hours for aggregate mode
    ...
    for year in years:
        ...
        if args.aggregate:
            # ── Aggregate mode: one parquet per month per run hour ──
            for month in months:
            ...
            # ── Normal mode: upload individual parquets ──
            for month in months:
                t0 = time.time()
