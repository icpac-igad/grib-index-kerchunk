...
|------|--------------|--------|---------|
| **1. Template** | One-time scan of two GRIB files (e.g. f000+f003) → builds a `.tar.gz` containing the deflated zarr store + per-member mapping parquets. The template is the *schema* of the dataset. | 2 sample GRIBs | `gik-fmrc-{model}-YYYYMMDD.tar.gz` (~10–30 MB) |
