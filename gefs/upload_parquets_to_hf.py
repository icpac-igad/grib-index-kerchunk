...
def _upload_with_retry(api, *, max_retries=5, **kwargs):
    ...
    HF caps repo commits at 128/hour. When the limit is hit the API returns
    HTTP 429; the only fix is to wait the rolling window. We back off
    starting at 5 min and double each retry.
    """
    delay = 300  # 5 min
...
def build_catalog(fs, api, years: list[str], dry_run: bool = False,
    ...
    # Catalog lives at the repo root (ECMWF convention) so users can grab it
    # with a single hf_hub_download(repo_id=..., filename="catalog.parquet").
...
def aggregate_month(fs, api, year: str, month: str, run_hour: str,
    ...
    try:
        ...
        for date_name, pf in all_parquets:
            ...
            n_rows = len(table)
            # Extract member from filename like "2024061500z-gep01.parquet"
            base = fname.rsplit(".parquet", 1)[0]
        ...
        # small row groups this enables PyArrow predicate pushdown on `date` /
        # `member`. A per-date query against a 250 MB monthly file then reads
        # only ~5–10 MB via HF range requests + parquet stats.
        combined = combined.sort_by([("date", "ascending"), ("member", "ascending")])
        out_path = os.path.join(tmpdir, "aggregated.parquet")
        pq.write_table(
            combined, out_path,
            row_group_size=60,    # ~30 members × 2 rows = one day per row group
